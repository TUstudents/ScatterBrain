...
try:
    ...
    from scatterbrain.visualization import (
        plot_iq,
    )  # plot_guinier, plot_porod, plot_fit (when implemented)
except ImportError:
...
def run_basic_workflow() -> None:
    ...
    if not data_file.exists():
        ...
        print(
            "Please ensure 'example_sphere_data.dat' is in an 'examples/data/' directory."
        )
        return
    ...
    try:
        ...
        curve = load_ascii_1d(
            data_file, q_col=0, i_col=1, err_col=2, skip_header=3, delimiter=r"\s+"
        )
        print(f"Loaded data: {curve}")
        print(f"  Number of points: {len(curve)}")
        print(f"  q range: {curve.q.min():.3g} - {curve.q.max():.3g} {curve.q_unit}")
        print(
            f"  Intensity range: {curve.intensity.min():.3g} - {curve.intensity.max():.3g} {curve.intensity_unit}"
        )
    ...
    # --- Plot Initial Data ---
    fig_data, ax_data = plot_iq(
        curve, title=f"Raw Data: {data_file.name}", errorbars=True
    )
    ...
    if guinier_results:
        ...
        print(
            f"    Rg            : {guinier_results['Rg']:.3f} +/- {guinier_results['Rg_err']:.3f} {curve.q_unit[:-3] if curve.q_unit.endswith('^-1') else 'units'}"
        )
        print(
            f"    I(0)          : {guinier_results['I0']:.3e} +/- {guinier_results['I0_err']:.3e} {curve.intensity_unit}"
        )
        print(
            f"    Fit q-range   : {guinier_results['q_fit_min']:.3g} - {guinier_results['q_fit_max']:.3g} {curve.q_unit}"
        )
        print(f"    Points used   : {guinier_results['num_points_fit']}")
        print(
            f"    R-value       : {guinier_results['r_value']**2:.4f} (R-squared)"
        )  # R-squared
    ...
    porod_results = porod_analysis(
        curve, q_fraction_high=0.3, fit_log_log=True
    )  # Auto high-q

    if porod_results:
        print("  Porod Fit Results (log-log):")
        print(
            f"    Porod Exponent: {porod_results['porod_exponent']:.3f} +/- {porod_results['porod_exponent_err']:.3f}"
        )
        print(
            f"    Porod Constant: {porod_results['porod_constant_kp']:.3e} +/- {porod_results['porod_constant_kp_err']:.3e}"
        )
        print(
            f"    Fit q-range   : {porod_results['q_fit_min']:.3g} - {porod_results['q_fit_max']:.3g} {curve.q_unit}"
        )
    ...
    initial_radius_guess = (
        guinier_results["Rg"] * np.sqrt(5 / 3)
        if guinier_results and not np.isnan(guinier_results["Rg"])
        else 4.0
    )
    initial_scale_guess = (
        guinier_results["I0"]
        if guinier_results and not np.isnan(guinier_results["I0"])
        else 1e5
    )
    initial_background_guess = np.median(
        curve.intensity[curve.q > curve.q.max() * 0.8]
    )  # Median of last 20% of I
    ...
    print(
        f"  Initial guesses for sphere fit: Scale={initial_scale_guess:.2e}, BG={initial_background_guess:.2f}, Radius={initial_radius_guess:.2f}"
    )  # Add print

    # Bounds: ([scale_low, bg_low, r_low], [scale_high, bg_high, r_high])
    # Adjust radius upper bound to be more accommodating or dynamic
    radius_upper_bound = max(
        20.0, initial_radius_guess * 1.5 + 5.0
    )  # Make upper bound more dynamic
    param_bounds_fit = (
        [1e2, 0, 0.5],  # Lower bounds
        [
            1e7,
            max(50.0, initial_background_guess * 2 + 10),
            radius_upper_bound,
        ],  # Upper bounds, also make BG upper bound more dynamic
    )
    print(
        f"  Parameter bounds for sphere fit: Lower={param_bounds_fit[0]}, Upper={param_bounds_fit[1]}"
    )  # Add print
    ...
    if sphere_fit_results and sphere_fit_results["success"]:
        ...
        print(
            f"    Fitted Background: {fp['background']:.3f} +/- {fpe['background']:.2f}"
        )
        print(
            f"    Fitted Radius   : {fp['radius']:.3f} +/- {fpe['radius']:.2f} {curve.q_unit[:-3] if curve.q_unit.endswith('^-1') else 'units'}"
        )
        print(f"    Reduced Chi^2   : {sphere_fit_results['chi_squared_reduced']:.3f}")
        print(
            f"    Fit q-range     : {sphere_fit_results['q_fit_min']:.3g} - {sphere_fit_results['q_fit_max']:.3g}"
        )
