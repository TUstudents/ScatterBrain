...
class TestConvertQArray:
    ...
    def q_values_a_inv(self) -> np.ndarray:
        ...
        return np.array(
            [1.0, 10.0, 100.0]
        )  # Corresponding values in A^-1 (0.1 nm^-1 = 1 A^-1)

    def test_nm_inv_to_a_inv(
        self, q_values_nm_inv: np.ndarray, q_values_a_inv: np.ndarray
    ):
    ...
    def test_a_inv_to_nm_inv(
        self, q_values_a_inv: np.ndarray, q_values_nm_inv: np.ndarray
    ):
    ...
    def test_same_unit_conversion(self, q_values_nm_inv: np.ndarray):
        ...
        converted_q_nm = convert_q_array(
            q_values_nm_inv, Q_NANOMETER_INV, Q_NANOMETER_INV
        )
    ...
    def test_string_variations(
        self, q_values_nm_inv: np.ndarray, current_unit: str, target_unit: str
    ):
    ...
    def test_unsupported_current_unit(self, q_values_nm_inv: np.ndarray):
        ...
        with pytest.raises(
            ValueError, match=re.escape("Unsupported current_unit 'm^-1'")
        ):
    ...
    def test_unsupported_target_unit(self, q_values_nm_inv: np.ndarray):
        ...
        with pytest.raises(
            ValueError, match=re.escape("Unsupported target_unit 'km^-1'")
        ):
...
class TestCustomExceptions:
    ...
    @pytest.mark.parametrize(
        "CustomError", [ProcessingError, AnalysisError, FittingError]
    )
